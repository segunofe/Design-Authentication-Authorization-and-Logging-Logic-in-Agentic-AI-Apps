...
# SEC307 Workshop - Activity 3a Generator Script
# Updates agent.py with the Activity 3a MCP client configuration.
#
# Security note: this script NO LONGER fetches or embeds the Cognito client
# secret. The Sales MCP client's credentials live in the AgentCore Identity
...
# Define file paths
SNIPPET_FILE="$SCRIPT_DIR/snippets/activity_3a.snip"
AGENT_FILE="$SCRIPT_DIR/agent.py"
TEMP_FILE="$SCRIPT_DIR/temp_activity3a.tmp"

print_status "Activity 3a Generator Script"
...
print_status "Step 2: Fetching the Sales OAuth2 credential provider name from CloudFormation..."
...
OAUTH_PROVIDER_NAME=$(aws cloudformation describe-stacks \
    ...
    --query "Stacks[0].Outputs[?OutputKey=='SalesMCPOAuthProviderName'].OutputValue" \
    --output text \
    --region "$AWS_REGION" 2>&1)

if [ $? -ne 0 ] || [ -z "$OAUTH_PROVIDER_NAME" ] || [ "$OAUTH_PROVIDER_NAME" == "None" ]; then
    print_error "Failed to fetch SalesMCPOAuthProviderName from $BOOTSTRAP_STACK"
...
print_success "Sales OAuth2 credential provider: $OAUTH_PROVIDER_NAME"

# Step 3: Process the snippet file and replace placeholders
print_status "Step 3: Processing snippet file..."
sed -e "s|###ACTIVITY3A_OAUTH_PROVIDER_NAME###|$OAUTH_PROVIDER_NAME|g" \
    -e "s|###ACTIVITY3A_GATEWAY_URL###|$GATEWAY_URL|g" \
...
awk '
BEGIN { in_activity3a = 0; replacement_done = 0 }
...
if grep -q "START OF ACTIVITY 3a" "$AGENT_FILE" && grep -q "END OF ACTIVITY 3a" "$AGENT_FILE"; then
    print_success "Activity 3a markers are intact"
else
    print_error "Activity 3a markers missing - file may be corrupted"
    print_warning "Restoring from backup..."
    mv "$AGENT_FILE.backup" "$AGENT_FILE"
    exit 1
fi

print_success "Activity 3a configuration completed successfully!"
