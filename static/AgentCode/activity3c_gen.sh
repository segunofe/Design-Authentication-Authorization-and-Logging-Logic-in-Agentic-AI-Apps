...
# SEC307 Workshop - Activity 3c Generator Script
# This script updates the agent.py file with Activity 3c configuration
...
# Define file paths
SNIPPET_FILE="$SCRIPT_DIR/snippets/activity_3c.snip"
AGENT_FILE="$SCRIPT_DIR/agent.py"
TEMP_FILE="$SCRIPT_DIR/temp_activity3c.tmp"

print_status "Activity 3c Generator Script"
...
awk '
BEGIN { in_activity3c = 0; replacement_done = 0 }
...
if grep -q "Customer Reviews retrieval" "$AGENT_FILE"; then
    print_success "Activity 3c content successfully inserted into agent.py"
else
    print_warning "Activity 3c content not found in updated file - please verify manually"
fi

# Check if Activity 3c markers are still present
if grep -q "START OF ACTIVITY 3c" "$AGENT_FILE" && grep -q "END OF ACTIVITY 3c" "$AGENT_FILE"; then
    print_success "Activity 3c markers are intact"
else
    print_error "Activity 3c markers missing - file may be corrupted"
    print_warning "Restoring from backup..."
    mv "$AGENT_FILE.backup" "$AGENT_FILE"
    exit 1
fi

print_success "Activity 3c configuration completed successfully!"
print_status "Summary:"
echo "  - Updated file: $AGENT_FILE"
echo "  - Backup file: $AGENT_FILE.backup"
echo "  - Content: Customer Reviews retrieval system prompt enhancement"
echo ""
